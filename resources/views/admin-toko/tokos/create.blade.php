...
@section('title', 'Tambah Toko - E-Canteen')
...
<div class="max-w-lg mx-auto px-4 sm:px-6 lg:px-8 pb-10">
    ...
    <h1 class="font-black text-2xl mt-1 mb-6">Tambah Toko Baru</h1>

    <div class="bg-white rounded-3xl border border-neutral-100 p-6">
        <form method="POST" action="{{ route('admin-toko.tokos.store') }}">
            @csrf
            <div class="mb-5">
            ...
            <div class="mb-5">
                ...
                <select name="user_id" id="user_id"
                    ...
                    <option value="{{ $penjual->id }}" {{ old('user_id') == $penjual->id ? 'selected' : '' }}
                        {{ $penjual->toko ? 'disabled' : '' }}>
                        {{ $penjual->name }} ({{ $penjual->email }}){{ $penjual->toko ? ' — sudah punya toko' : '' }}
                    </option>
                    @endforeach
                </select>
                @if($penjuals->isEmpty())
