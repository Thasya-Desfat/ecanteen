...
<div class="max-w-2xl mx-auto px-4 sm:px-6 lg:px-8 pb-10">
    ...
    <h1 class="font-black text-2xl mt-1 mb-6">Edit Menu</h1>

    <div class="bg-white rounded-3xl border border-neutral-100 p-6">
        <form method="POST" action="{{ route('admin-toko.tokos.menus.update', [$toko, $menu]) }}" enctype="multipart/form-data">
            @csrf
            @method('PUT')
            ...
            <div class="mb-4">
                ...
                <div class="relative">
                    ...
                    <input type="text" id="harga_display" value="{{ number_format(old('harga', $menu->harga), 0, ',', '.') }}"
                ...
                <input type="hidden" name="harga" id="harga" value="{{ old('harga', $menu->harga) }}">
                @error('harga')
                <p class="text-red-500 text-xs font-bold mt-1">{{ $message }}</p>
                @enderror
            </div>
            ...
            <div class="mb-4">
                @if($menu->foto)
                <div class="mb-3">
                    <img src="{{ asset('storage/' . $menu->foto) }}" alt="{{ $menu->nama_menu }}" class="w-28 h-28 object-cover rounded-2xl border border-neutral-100">
                </div>
                @endif
                <label for="foto" class="block text-xs font-black uppercase tracking-widest text-neutral-400 mb-2">Foto Menu <span class="text-neutral-300 normal-case font-normal">(kosongkan jika tidak ingin mengubah)</span></label>
